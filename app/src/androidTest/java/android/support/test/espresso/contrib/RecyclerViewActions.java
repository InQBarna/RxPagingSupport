...
public final class RecyclerViewActions {
    ...
    private static Matcher<? super View> isRecyclerViewWithLayoutManager() {
        return new TypeSafeMatcher<View>() {
            @Override
            protected boolean matchesSafely(View item) {
                RecyclerView.LayoutManager lm = getLayoutManager(item);
                return lm != null;
            }

            private RecyclerView.LayoutManager getLayoutManager(View item) {
                if (!(item instanceof RecyclerView)) {
                    return null;
                }
                return ((RecyclerView) item).getLayoutManager();
            }

            @Override
            protected void describeMismatchSafely(View item, Description mismatchDescription) {
                mismatchDescription.appendText("was a ").appendValue(item.getClass().getName())
                                   .appendText(" with layoutManager = ").appendValue(getLayoutManager(item));
            }

            @Override
            public void describeTo(Description description) {
                description.appendText("is RecycleView with valid layoutmanager");
            }
        };
    }
    ...
    private static final class ScrollToPositionViewAction implements ViewAction {
        ...
        @Override
        public Matcher<View> getConstraints() {
            return allOf(isRecyclerViewWithLayoutManager(), isDisplayed());
        }
